...
1. 你能整体讲一下会员营销前端的架构吗？应用仓库和基础组件仓库分别承担什么职责？
    - 应用仓库，负责具体业务场景的实现，包括页面布局、业务逻辑、数据获取等。
    - 基础组件仓库，它包含了构成页面的所有基础元素和渲染它们的核心引擎。比如通用的 UI 组件、Bricks渲染引擎、表单系统、dsl转换等。
    优势
...
2. DSL → UI 的完整渲染链路是怎样的？从后端拿到一段 JSON，到前端渲染出页面，中间经历了哪些关键步骤和模块？等）？
    ...
    阶段二：组件递归渲染（在 bricks-render 包中）
        ...
        3. 实例化与渲染：将 DSL 节点中的 data (此时已包含 Adapter 转换好的数据) 作为 props 传递给找到的 React 组件，并使用 React.createElement 创建实例。
...
五、Lynx & 动态化（member-lynx / member-lynx-dynamic）
...
2. 简单说一下一个典型的开发/构建流程：从改一行代码，到本地启动调试，再到打包构建、CI 发布，中间都有哪些关键步骤和工具？踩坑？
    ...
    1. 开发阶段：
    - git pull 拉取最新代码。
    - 在根目录执行 pnpm i，emo 会自动处理所有包的依赖安装和内部软链接。
...
3. 如果需要在 DSL 层做数据依赖（比如组件 B 依赖组件 A 的数据），这个项目里是怎么设计和实现的？
    这是一个很高级的场景，比如页面上有个城市选择器（组件 A），当用户选择城市后，下方的门店列表（组件 B）需要根据选择的城市，重新请求数据并刷新。
